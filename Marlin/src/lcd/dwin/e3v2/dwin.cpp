  ...
  void HMI_Move_E() {
    ...
    if (encoder_diffState != ENCODER_DIFF_NO) {
      if (Apply_Encoder(encoder_diffState, HMI_ValueStruct.Move_E_scaled)) {
        ...
        DWIN_Draw_Signed_Float(font8x16, Color_White, Color_Bg_Black, 3, UNITFDIGITS, 216, MBASE(4), HMI_ValueStruct.Move_E_scaled);
        if (!planner.is_full()) {
      ...
      DWIN_Draw_Signed_Float(font8x16, Color_White, Select_Color, 3, UNITFDIGITS, 216, MBASE(4), HMI_ValueStruct.Move_E_scaled);
      DWIN_UpdateLCD();
    }
  }
  ...
  */
  ...
  /* Bottom nozzle temperature update, originalY=382 */
    ...
    if (_hotendtemp != thermalManager.temp_hotend[0].celsius) {
      DWIN_Draw_IntValue(true, true, 0, DWIN_FONT_STAT, DWIN_Temp_Colorize(thermalManager.temp_hotend[0].celsius),
                         Color_Bg_Black, 3, 33, Status_Row_1, thermalManager.temp_hotend[0].celsius);
  ...
  /* Bottom bed temperature update, originalY=382 */
    ...
    if (_bedtemp != thermalManager.temp_bed.celsius) {
      DWIN_Draw_IntValue(true, true, 0, DWIN_FONT_STAT, DWIN_Temp_Colorize(thermalManager.temp_bed.celsius),
                         Color_Bg_Black, 3, 178, Status_Row_1, thermalManager.temp_bed.celsius);
    ...
    switch (select_prepare.now) {
      ...
      case PREPARE_CASE_MOVE: // Axis move
        ...
        DWIN_Draw_FloatValue(true, true, 0, font8x16, Color_White, Color_Bg_Black, 3, UNITFDIGITS, 216, MBASE(3), current_position.z * MINUNITMULT);
          ...
          DWIN_Draw_Signed_Float(font8x16, Color_White, Color_Bg_Black, 3, 1, 216, MBASE(4), HMI_ValueStruct.Move_E_scaled);
        #endif
        break;
      ...
      case 3: // Z axis move
        ...
        break;
          ...
          case 4: // Extruder
            ...
            checkkey = Extruder;
            HMI_ValueStruct.Move_E_scaled = current_position.e * MINUNITMULT;
            DWIN_Draw_Signed_Float(font8x16, Color_White, Select_Color, 3, 1, 216, MBASE(4), HMI_ValueStruct.Move_E_scaled);
            EncoderRate.enabled = true;
            break;
        #endif
    }
